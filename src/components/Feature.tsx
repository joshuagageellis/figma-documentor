...
export const FeatureCard = ({
  ...
  const handleDelete = () => {
		...
		if (confirmDelete) {
			setFeatures((state: Feature[]) => state.filter((f) => f.id !== feature.id));
		}
  };
  ...
  const handleTagNode = (target: 'image' | 'document') => {
	...
	const handleUntagTextNode = (id: string) => {
		setFeatures((state) =>
			state.map((f) =>
				f.id === feature.id ? { ...f, notes: f.notes.filter((n) => n.nodeId !== id) } : f
			)
		);
	};
  ...
  return (
    <article className="feature shadow-sm rounded-md p-2">
			<AnimatePresence>
				<div key={feature.id + '-header'} className='flex flex-row gap-4 items-center justify-between h-12'>
					{isCollapsed ? (
						<h2 className="text-lg font-semibold">{feature.title.length > 0 ? (feature.title) : (<span className='text-gray-500'>[Untitled]</span>)}</h2>
					) : (
						<input
							type="text"
							value={feature.title}
					...
					<div className='flex flex-row gap-4'>
						...
						<Button onClick={() => setIsCollapsed(!isCollapsed)} type={isCollapsed ? 'plus' : 'minus'}>
								...
								{feature.notes.map((note, index) => (
									<div key={`note-${index}`} className="flex flex-col gap-2">
										<div className="flex flex-row gap-2 w-full justify-between">
											...
											<Button onClick={() => handleUntagTextNode(note.nodeId)} type="minus">
												Delete
											</Button>
										</div>
								...
								{feature.images.map((imgNode, index) => (
									<div key={`img-${index}`} className="flex flex-col gap-2">
										<div className="flex flex-row gap-2 w-full justify-between">
											...
											<Button onClick={() => handleImageDelete(index)} type="minus">
												Delete
											</Button>
										</div>
										<iframe src={imgNode.embedUrl} className="w-full h-auto"></iframe>
									</div>
								))}
							</div>
							<Button onClick={() => handleTagNode('image')} type="plus" disabled={nodeCount === 0}>
								Tag New Frame
							</Button>
						</div>
					</div>
